...
public class BingSearch {
	...
	private VSMVector firstResults = new VSMVector();
	
	public BingSearch(StopList stoplist){
		this.stoplist = stoplist;
	}

	
	public void execute(String initialQuery) throws MalformedURLException, IOException, SAXException{
		Page page = downloadResults("%27" + initialQuery + "%27");
...
//					System.out.println("URL:" + urls[j]);
					...
					if(page != null){
						VSMVector result = new VSMVector(page.getContent(),stoplist,false);
						result.normalize();
						if(i == 0){
								firstResults.addVector(result);
								positiveTemp.addVector(result);
								count++;
						}else{
							double sim = firstResults.vectorSpaceSimilarity(result);
							System.out.println("SIM:" + sim);
							boolean relevant = firstResults.vectorSpaceSimilarity(result) > 0.5;
							if(relevant){
...
//								positiveTemp1.normalize();
								positiveTemp.addVector(result);
//								System.out.println("###" + positiveTemp.size());
//								VSMElement[] elems = positiveTemp.topElements(positiveTemp.getArrayElements().length);
//								System.out.println("###");
//								for (int k = 0; k < elems.length && k < 10; k++) {
//									System.out.print(elems[k].getWord() + ":" + elems[k].getWeight());
//								}
//								System.out.println("");
								count++;
							}else{
...
//								negativeTemp1.normalize();
	...
	private Page downloadResults(String keyword){
		...
		try {
			url = new URL("https://api.datamarket.azure.com/Data.ashx/Bing/SearchWeb/v1/Web?Query=" + keyword + "&$top="+ numberOfResults);
...
//	         }
	...
	private String nextQuery(double currentPrecision, int currentUnique, String currentQuery, VSMVector pos, VSMVector neg){
		...
		double proportionUnique = (double)currentUnique/(double)numberOfResults;
		if(proportionUnique <= 0.5){
			changeQuery = true;
		}
		if(!changeQuery && (offsetCounter/numberOfResults) +1 <= 6){
			...
			offsetCounter = offsetCounter+numberOfResults;
			
			return "%27" + previousQuery + "%27" + offset + offsetCounter;
		}
		offsetCounter = 1;
		int index = currentQuery.indexOf(offset);
		String tempQuery = "";
...
//		}
		...
		previousQuery = currentQuery;
		return "%27" + query.toString() + "%27";
	}

	public static void main(String[] args) {
		try {
			StopList stoplist = new StopListArquivo(args[0]);
			BingSearch search = new BingSearch(stoplist);
