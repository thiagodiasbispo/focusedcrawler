...
//			OutputStream fout = new FileOutputStream(args[1],true);
//	        Writer output = new OutputStreamWriter(fout, "UTF8");
	        

//			OutputStream fout= new FileOutputStream(args[1],true);
//			OutputStream bout= new BufferedOutputStream(fout);
//			OutputStreamWriter output = new OutputStreamWriter(bout);

//			File file = new File(args[0]);
//			File[] files = file.listFiles();
//			for (int i = 0; i < files.length; i++) {
//				System.out.println(files[i].getName());
//				if(files[i].getName().contains(".wmv_") || files[i].getName().contains(".zip_") 
//						|| files[i].getName().contains(".xls_") || files[i].getName().contains(".png_")
//						|| files[i].getName().contains(".pptx_") || files[i].getName().contains(".doc_")
//						|| files[i].getName().contains(".tif_") || files[i].getName().contains(".mp3_")
//						|| files[i].getName().contains(".exe_") || files[i].getName().contains(".xml_")
//						|| files[i].getName().contains(".ashx") || files[i].getName().contains(".bmp_")
//						|| files[i].getName().contains(".ppt_") || files[i].getName().contains(".mov_")
//						|| files[i].getName().contains(".msi_")){
//					continue;
//				}
...
//				s = new String(buf);
				...
				Page page = universalDownloader.downloadPage("http://brasil.estadao.com.br/blogs/estadao-rio/mais-de-3-000-motoristas-podem-perder-a-cnh-no-rio/");
				System.out.println(page.getEncoding());
				Html2Txt h = new Html2Txt();
				String[] sentences = h.convert(page.getContent()); 
				for (int j = 0; sentences != null && j < sentences.length; j++) {
					if(!sentences[j].equals("")){
						sentences[j] = sentences[j].replace("*", "");
						String[] words = sentences[j].split(" ");
						if(words.length > 4){
							System.out.println(sentences[j] + "\n");
//							output.write(sentences[j] + "\n");
						}
					}
				}
//				output.flush();
